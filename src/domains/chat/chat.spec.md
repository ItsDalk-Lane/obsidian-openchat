...
- 预期结果：
  ...
  - 选中“提示模板”后，关闭 `@` 菜单，并在 `@` 符号锚点位置打开一个小型模板下拉菜单；该菜单列出
    ...
    直接调用 `selectPromptTemplate()` 应用模板；后续发送消息时，模板内容始终作为用户消息任务模板参与组装，
    不提供切换为系统提示词的入口。
...
- 边界情况：
  ...
  ### 行为 16：在输入框中通过拖拽与粘贴添加附件

  - 触发条件：consumer 将文件、文件夹或图片拖入 chat 输入框，或在输入框中粘贴图片/文件。
  - 预期结果：
    - 当拖入内容包含文件负载时，输入框外层容器进入明确的 drag-over 视觉态；drop 或 dragleave 后立即恢复。
    - 图片会转成 base64 并加入 `selectedImages`；文本类附件会加入 `selectedFiles`。
    - 当拖入文件夹时，应递归收集其中所有受支持文件，并按文件批量加入 `selectedFiles`，而不是新增
      `selectedFolders` 条目。
    - 当拖入/粘贴的是当前 Vault 内已有文件时，应复用原 vault 路径；当来源位于 Vault 外部时，应先导入到
      `System/AI Data/chat-history/files` 再加入附件上下文。
    - 当外部导入的文本附件在输入区被移除时，应同步删除对应的受管导入文件；若附件原本就在当前 Vault 内，则不
      应删除原文件。
    - 当包含外部导入文本附件的聊天历史记录被删除时，应同步删除该历史记录 frontmatter 中登记的受管导入文件。
    - 对不支持的扩展名或处理失败项，应通过 Notice 给出汇总提示，但不清空输入内容，也不丢失输入焦点。
  - 边界情况：
    - 当目标文件路径已存在于 `selectedFiles` 中时，应静默跳过，不重复添加。
    - 当粘贴内容不包含图片、文件或可解析的文件路径时，应保留默认文本粘贴行为。
    - 当一次批量操作同时包含成功项与失败项时，Notice 应同时报告已添加数量与失败/不支持数量。
    - 当拖入内容来自文件管理器且包含嵌套子目录时，应继续递归处理所有层级中的受支持文件。
