...
- **不要用于发现路径或浏览目录**：请改用 \`find_paths\`、\`list_directory_flat\`、\`list_directory_tree\` 或 \`list_vault_overview\`
...
export const LIST_DIRECTORY_DESCRIPTION = `兼容型目录浏览工具；当 wrapper surface 已启用时，优先使用 \`list_directory_flat\`、\`list_directory_tree\` 或 \`list_vault_overview\`。该工具支持 flat、tree 和 vault 三种视图。
...
- 在调用 \`read_file\`、\`open_file\`、\`list_directory_flat\`、\`list_directory_tree\` 或 \`list_vault_overview\` 之前，需要先定位准确路径时

## 何时不使用

- **不要用于读取内容**：读文件请使用 \`read_file\`
- **不要用于浏览已知目录**：浏览目录请使用 \`list_directory_flat\`、\`list_directory_tree\` 或 \`list_vault_overview\`
- **不要用于正文搜索**：按内容搜索请使用 \`search_content\`
