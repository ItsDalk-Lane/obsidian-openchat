...
async function checkCwdAllowed(cwd: string): Promise<ApiResponse | null> {
...
export async function POST(req: Request) {
  try {
    ...
    if (!body.branch || typeof body.branch !== "string") {
      return ApiResponse.json({ error: "branch is required" }, { status: 400 });
    }
    const denied = await checkCwdAllowed(body.cwd);
    if (denied) return denied;
    if (!existsSync(body.cwd)) {
      return ApiResponse.json({ error: `Directory does not exist: ${body.cwd}` }, { status: 400 });
    }
...
export async function DELETE(req: Request) {
  try {
    ...
    return ApiResponse.json({ error: message, dirty }, { status: dirty ? 409 : 400 });
  }
}
