...
- 实测 `rg -n "app/api" --glob '*.test.mjs' lib components` 命中 9 个测试文件：其中既有 `readFile(new URL("../app/api/..."))` 源码断言，也有 `jiti.import("../app/api/...")` 运行时导入。
- 一旦物理删除 `app/`，这些测试必然因文件不存在而失败；保留兼容目录或链接又不满足“`app/` 整体删除”。在既定白名单内没有第三处可修复。
