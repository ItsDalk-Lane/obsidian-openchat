...
test("extension command reload syncs project trust first", async () => {
  const { ExtensionUiBridge } = await jiti.import("./rpc/extension-ui-bridge.ts");
  const calls = [];
  const bridge = new ExtensionUiBridge({
    inner: {
    ...
    prepareReload: () => { calls.push("trust"); },
  });

  await bridge.createCommandContextActions().reload();
  assert.deepEqual(calls, ["trust", "reload", "bind", "prompt"]);
});
