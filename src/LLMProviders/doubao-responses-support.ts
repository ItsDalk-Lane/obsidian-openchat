...
import { createDoubaoHTTPError, extractString } from './doubaoUtils'
import type {
	DoubaoOptions,
...
export const normalizeDoubaoThinkingType = (
...
export const normalizeDoubaoReasoningEffort = (
...
const buildWebSearchTool = (
	...
	if (
		typeof webSearchConfig?.maxKeyword === 'number'
		&& webSearchConfig.maxKeyword > 0
	) {
...
export const resolveToolExecutor = (
...
const resolveLegacyMcpTools = async (
	options: DoubaoOptions,
): Promise<ToolDefinition[]> => {
	const legacyTools = await resolveCurrentMcpTools(
		options.mcpTools,
		options.mcpGetTools,
	)
...
export const extractResponseFunctionCallsForLoop = (
	response: unknown,
...
export const mapResponseCallsToToolLoop = (
...
export const buildFunctionOutputItems = (
	calls: ResponseFunctionCall[],
...
export const buildBaseResponsesParams = (
	options: DoubaoOptions,
	...
	if (
		...
		&& typeof mapped.max_output_tokens !== 'number'
	) {
...
export const buildRequestData = (params: {
	model: string
	input: unknown
...
export const streamResponseRound = async function* (
	response: Response,
...
export const isUnsupportedDoubaoResponsesModel = (model: string): boolean =>
	DOUBAO_UNSUPPORTED_RESPONSES_MODELS.has(model.trim().toLowerCase())
