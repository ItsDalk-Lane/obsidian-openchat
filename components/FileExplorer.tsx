...
async function fetchGitStatus(cwd: string): Promise<GitStatusResponse> {
  ...
  if (!res.ok) throw new Error(`加载 Git 状态失败 (HTTP ${res.status})`);
  ...
  return (
    <div style={{ minHeight: "100%" }}>
      ...
      {showUploadFeedback && (
        ...
        {uploadBusy && (
          <div role="status" aria-live="polite" aria-label={uploadPhase === "checking" ? "检查文件中" : `上传中，${uploadProgress}%`}>
            ...
            <div style={{ display: "flex", alignItems: "center", gap: 8, minHeight: 22, fontSize: 11 }}>
              <div style={{ minWidth: 0, flex: 1, display: "flex", alignItems: "center", gap: 8 }}>
                {uploadSummary.uploaded.length > 0 && (
                  <span title={`${uploadSummary.uploaded.length} 已上传`} aria-label={`${uploadSummary.uploaded.length} 已上传`} style={{ display: "flex", alignItems: "center", gap: 3, color: "#22c55e" }}>
                ...
                {uploadSummary.skipped.length > 0 && (
                  <span title={`${uploadSummary.skipped.length} 已跳过`} aria-label={`${uploadSummary.skipped.length} 已跳过`} style={{ display: "flex", alignItems: "center", gap: 3, color: "var(--text-dim)" }}>
                ...
                {uploadSummary.errors.length > 0 && (
                  <span title={`${uploadSummary.errors.length} 失败`} aria-label={`${uploadSummary.errors.length} 失败`} style={{ display: "flex", alignItems: "center", gap: 3, color: "#f87171" }}>
                    <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" aria-hidden="true">
      ...
      <div style={{ padding: "2px 4px" }}>
        {loading ? (
          <div style={{ padding: "8px 12px", fontSize: 11, color: "var(--text-dim)" }}>加载文件中...</div>
        ) : error ? (
