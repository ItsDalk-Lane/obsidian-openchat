...
export const editMessage = async (
	...
	try {
		...
		deps.notify(localInstance.chat_file_update_failed_message_edited);
	}
};
...
export const deleteMessage = async (
	...
	try {
		...
		deps.notify(localInstance.chat_file_update_failed_message_deleted);
	}
};
...
export const togglePinnedMessage = async (
	...
	try {
		...
		deps.notify(localInstance.chat_pinned_state_sync_failed);
	}
};
...
export const insertMessageToEditor = (
	...
	const insertResult = deps.insertTextIntoMarkdownEditor(message.content);
	if (insertResult.inserted && !insertResult.fileName) {
		deps.notify(localInstance.chat_inserted_current_editor);
		return;
	}

	if (insertResult.inserted) {
		const fileName = insertResult.fileName ?? localInstance.chat_unknown_file;
		deps.notify(localInstance.chat_inserted_file.replace('{fileName}', fileName));
		return;
	}

	deps.notify(localInstance.chat_insert_target_missing);
};
...
export const regenerateFromMessage = async (
	...
	if (target.role !== 'assistant') {
		deps.notify(localInstance.chat_regenerate_only_ai_message);
		return;
	}
