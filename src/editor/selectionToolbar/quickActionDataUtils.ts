...
export interface OpenChatPluginLike {
...
export function normalizeQuickActions(rawList: unknown[], promptResolver?: (item: RawQuickAction, index: number) => string): QuickAction[] {
	const now = Date.now();
	return rawList
...
export function parseMarkdownRecord(content: string): { frontmatter: RawQuickAction; body: string } {
...
export function buildMarkdownRecord(frontmatter: RawQuickAction, body: string): string {
