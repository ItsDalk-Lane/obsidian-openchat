...
import type { ChatAttachmentSelectionService } from './ChatAttachmentSelectionService';
...
export const prepareChatRequest = async (
	...
	if (deps.state.isGenerating) {
		new Notice(localInstance.chat_request_in_progress);
		return null;
	}
	...
	if (deps.state.shouldSaveHistory) {
		if (
			...
			try {
				...
				new Notice(localInstance.chat_session_file_create_failed_but_sent);
			}
		} else {
			try {
...
export const sendMessage = async (
	...
	if (deps.state.multiModelMode === 'compare') {
		if (!deps.multiModelService) {
			new Notice(localInstance.chat_multi_model_service_not_initialized);
			return;
		}
