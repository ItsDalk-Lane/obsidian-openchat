...
export class StatusBarManager {
	...
	constructor(
		...
		this.state = {
			...
			content: {
				text: 'OpenChat',
				tooltip: t('OpenChat AI assistant is ready')
			},
			timestamp: new Date()
		}
	...
	updateGeneratingProgress(characters: number) {
		...
		this.updateState({
			content: {
				text: `OpenChat: ${characters}${t('characters')}`,
	...
	setSuccessStatus(stats: GenerationStats) {
		...
		this.updateState({
			type: 'success',
			content: {
				text: `OpenChat: ${stats.characters}${t('characters')} ${stats.duration}`,
	...
	setCancelledStatus() {
		...
		this.updateState({
			type: 'idle',
			content: {
				text: `⚠️ OpenChat: ${t('Generation cancelled')}`,
