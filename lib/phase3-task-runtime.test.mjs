...
test("a native task can own multiple Pi runs and resolver routes return durable task ids", async (t) => {
  ...
  const { GET: resolveRoute } = await jiti.import("../server/api/tasks/resolve/route.ts");
...
test("task detail and artifact routes enforce TaskId semantics and file-access checks", async (t) => {
  ...
  const detailRoute = await jiti.import("../server/api/tasks/[id]/route.ts");
  ...
  const artifactRoute = await jiti.import("../server/api/tasks/[id]/artifacts/route.ts");
