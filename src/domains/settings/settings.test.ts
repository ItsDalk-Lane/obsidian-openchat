...
test('PluginSettingsController loadSettings 会返回设置并同步调试适配器', async () => {
	installTestWindow();
	const { DEFAULT_SETTINGS } = await import('./config');
	const { PluginSettingsController } = await import('./ui');
	const debugAdapter = createDebugAdapterSpy();
	...
	const controller = new PluginSettingsController({
		async load(): Promise<PluginSettings> { return expectedSettings; },
...
test('SettingsDomainService load 在空数据下回退到默认设置', async () => {
	installTestWindow();
...
test('SettingsDomainService load 在默认系统消息迁移失败时记录错误并继续返回设置', async () => {
	installTestWindow();
	const { SettingsDomainService } = await import('./service');
	const { DEFAULT_SETTINGS } = await import('./config');
...
test('SettingsDomainService load 会合并成功读取的 MCP 服务器列表', async () => {
	installTestWindow();
	const { SettingsDomainService } = await import('./service');
	const { DEFAULT_SETTINGS } = await import('./config');
	const settingsService = new SettingsDomainService({
...
test('SettingsDomainService load 会迁移默认系统消息并在 MCP 读取失败时回退空列表', async () => {
	installTestWindow();
	const { SettingsDomainService } = await import('./service');
	const { DEFAULT_SETTINGS } = await import('./config');
	const loggedMessages: string[] = [];
