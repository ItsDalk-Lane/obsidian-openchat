...
function createRuntimeFixture(options?: {
	...
	const runtime: ObsidianApiRuntime = {
		...
		async requestUrl(): Promise<{ readonly status: number; readonly text?: string; readonly headers: Record<string, string> }> {
			return {
				status: 204,
...
test('ObsidianApiProvider 规范化 HTTP 响应并为空文本兜底', async () => {
	const provider = createObsidianApiProviderFromRuntime(
		createRuntimeFixture({ requestText: undefined }).runtime,
		async () => '',
	);
