...
test("all project resource loaders enforce project trust", async () => {
  const factorySource = await readFile(
    new URL("./adapters/pi/pi-session-factory.ts", import.meta.url),
    "utf8",
  );
  const rpcSource = await readFile(new URL("./rpc-manager.ts", import.meta.url), "utf8");
  const modelsSource = await readFile(new URL("../server/api/models/route.ts", import.meta.url), "utf8");
  const skillsSource = await readFile(new URL("./skills-service.ts", import.meta.url), "utf8");
  const skillsInstallSource = await readFile(
    new URL("../server/api/skills/install/route.ts", import.meta.url),
    "utf8",
  );
  const pluginsSource = await readFile(new URL("../server/api/plugins/route.ts", import.meta.url), "utf8");

  assert.match(factorySource, /resourceLoaderReloadOptions: projectTrustReloadOptions\(input\.cwd, agentDir\)/);
  assert.match(rpcSource, /syncProjectTrust: \(\) => this\.syncProjectTrust\(\)/);
  assert.match(rpcSource, /prepareReload: \(\) => this\.syncProjectTrust\(\)/);
  assert.match(modelsSource, /resourceLoaderReloadOptions: projectTrustReloadOptions\(cwd, agentDir\)/);
  assert.match(skillsSource, /loader\.reload\(projectTrustReloadOptions\(cwd, agentDir\)\)/);
  assert.match(pluginsSource, /projectTrusted: trustStatus\.trusted/);
  assert.match(skillsInstallSource, /getProjectTrustStatus\(cwd, getAgentDir\(\)\)\.trusted/);
  assert.match(pluginsSource, /scope === "project" && !trustStatus\.trusted/);
});

test("the trust API invalidates cached models and same-project runtimes", async () => {
  const source = await readFile(
    new URL("../server/api/project-trust/route.ts", import.meta.url),
    "utf8",
  );
  const rpcSource = await readFile(new URL("./rpc-manager.ts", import.meta.url), "utf8");
  ...
  assert.match(rpcSource, /trackStartingSessionCwd\(cwd\)/);
});
