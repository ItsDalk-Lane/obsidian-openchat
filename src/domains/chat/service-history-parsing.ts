...
import type { ChatMessage, SubAgentExecutionState, ToolCall } from './types'
...
export const parseSubAgentStatesFromHistory = (
	...
	const subAgentStates: Record<string, SubAgentExecutionState> = {}
	...
	while ((match = calloutPattern.exec(content)) !== null) {
		let status: SubAgentExecutionState['status'] = 'completed'
...
const parseToolCallsFromCallout = (content: string): {
	...
	const parseBlock = (blockLines: string[]) => {
		...
		for (const rawLine of blockLines) {
			...
			const headerLine = line.startsWith('[!info]- ')
				? line.replace('[!info]- ', '')
				: line
	...
	for (const line of lines) {
		...
		if (headerMatch) {
			if (inMessage && currentContent.trim()) {
				messages.push(
					createMessage(currentRole, currentContent.trim(), {
