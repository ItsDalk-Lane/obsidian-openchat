...
- [x] `DirectoryPicker` 与 `DeleteSessionDialog` 后续均通过开发服务直接加载当前真实组件、中文环境和全局样式完成浏览器亲验，不复制实现、不访问会话接口。前者实际切换隐藏目录列表 2→3，后者实际显示取消/仅删本会话/连同 3 个分支删除；语义、层级、换行、圆角和裁切均通过，控制台/页面错误为 0，截图在 `stage1-missing-modals/`。这证明真实组件视觉与局部交互，不冒充完整用户会话流。
...
- [x] 阶段一提交：`style: unify settings dialog visuals`；提交后工作区干净。
- [~] 一次性 `/tmp/pi-web-stage1-019fbb67` 清理命令被执行环境安全策略拒绝；未绕过限制，目录留给系统自动清理，仓库与用户数据未受影响。

### 阶段二进度

- [x] 固定拆分：无 Electron 依赖的窗口状态/通知判断纯逻辑与单测 → 主进程/预加载白名单接线 → 渲染端桌面桥接线；纯 Web 无桥时不增加行为。
- [x] 不使用 `globalShortcut`；菜单快捷键只通过应用菜单的 `CmdOrCtrl+N` 转发到当前窗口。
- [x] 源码基线：`electron/main.js` 507 行，窗口固定 1400×900；预加载白名单只有主题、目录选择、启动重试与强制打开；文件菜单当前仅关闭/退出。既有更新、加载页、主题、目录选择链路均有明确锚点。
- [x] 渲染端现有新会话入口最终汇合到 `AppShell` 的同一动作；完成声音已在 `ChatWindow` 的完成包装器中调用，桌面通知可在同一点追加桥调用，桥不存在时不做任何事。
- [x] 安全边界决策：预加载层只暴露“完成通知”和“新建会话监听”两个精确能力，不暴露原始消息通道或事件对象；预加载与主进程双层校验短文本，主进程另校验发送者就是当前窗口。通知不包含提示词、路径、会话名或模型输出。
- [x] 纯逻辑与测试已落在 `electron/lib/`：窗口状态支持坏 JSON、坏字段、缺文件、负坐标副屏、部分越界和完全丢屏；通知只在未聚焦且运行状态刚结束时为真。`node --test "electron/lib/*.test.cjs"` 实测 4/4 通过，0 fail/skip/todo；强制 ESLint、Electron 导入扫描与脚本语法检查均通过。
- [x] 接线完成：窗口位置/尺寸/最大化状态使用 300ms 防抖和同目录临时文件改名保存，启动时按实际多屏交叠选屏并收回可见区；页面隐藏且运行结束时发中英通知，点击恢复/显示/聚焦主窗口；菜单复用既有新会话动作。
- [x] Dock 角标仅为建议，本阶段不引入运行数量聚合和额外状态耦合；选用现有的统一完成入口，守住纯 Web 行为不变。
- [x] 阶段二反向验证：临时去掉尺寸上界收拢后，窗口状态测试退出 1，3 测中 2 过/1 败，明确报出 3000×2000 未收回 1920×1080；原样恢复后 `node --test "electron/lib/*.test.cjs"` 退出 0，4/4 通过、0 fail/skip/todo。
- [x] 接线后小范围验收：`node --check` 主进程/预加载脚本通过；类型检查和目标文件 lint 退出 0；i18n parity 为 en=452 / zh-CN=452 且零差异；`git diff --check` 无输出。
- [x] 阶段二全量检查保持基线：`npm run check` 的 typecheck/lint 通过，主测试仍为 280 测 / 278 过 / 仅 2 个既定具名失败 / 0 skipped / 0 todo；因 `&&` 停于既定失败后，单独复跑 Pi 适配器 11/11 全过。
- [x] 桌面打包验收：`npx electron-builder --mac dir` 退出 0，完成 arm64 原生模块重建与 `dist-electron/mac-arm64/Pi Web Desktop.app` 目录打包；按项目既定配置跳过签名。`grep -c "检查更新" electron/main.js` 为 5，`globalShortcut` 扫描为 0。
- [x] 阶段二收口服务验证：隔离数据目录下以页面 5174 / 接口 30142 启动 `npm run dev`；`node scripts/smoke-api.mjs` 15/15 通过、覆盖 67 路由；同一 5174 页面执行 visual-smoke，桌面/移动水平溢出均为 false，页面/控制台/请求错误均为 0，末行 `VISUAL SMOKE PASS`；截图在本任务可视化目录 `stage2/`。
- [x] 真实桌面目录包在全隔离的会话/运行/配置/用户数据目录下以端口 31987 启动：主进程、服务子进程、图形与页面进程均持续运行；首页与会话接口均返回 200；正常退出后端口释放、全部本轮进程回收。状态文件实际产生为 1400×900 / 未最大化，权限 `0600`；日志无 uncaught/fatal/crash/启动失败。目录包缺更新元数据导致后台更新检查警告，但应用未崩溃，不影响本阶段验收。
- [x] 最终桌面包的窗口与菜单链已补真实运行证据：状态文件保存 `x=120 / y=110 / 1120×760` 后普通窗口重启精确恢复；保存最大化再重启得到 `2560×1345 / maximized=true`，普通边界仍精确保持。应用菜单实际项为“新建会话”/`CmdOrCtrl+N`，调用该真实菜单项后预加载监听计数 1，页面地址从带 cwd 回到 `/`，既有输入框仍为 1。
- [x] 窗口亲验首轮抓到 macOS 首次显示会把已保存横坐标 120 级联成 244；已改为真实显示后再次落实已校验边界，再恢复最大化。同一最终包重测得到上述精确边界，详见 `BLOCKED.md` V8。
- [x] 完成通知的应用内链路已用最终桌面包和隔离假模型跑通：页面实际新建会话、发送消息，服务端收到流式请求并以 `stop` 结束；会话文件保存真实回复，持久事件表记录同一操作的 `operation.started` 与 `operation.completed / prompt_done`，页面显示回复且四类页面错误为 0。测试页临时注入隐藏状态后，完成回调经预加载桥和主进程来源校验调用原生通知 1 次，标题/正文正确；截图在 `stage2-desktop/real-agent-completion-chain.png`。该隐藏值是测试注入，不冒充操作系统真实后台状态。
- [待亲验] 当前 macOS 对上述真实任务通知返回 `show=0 / failed=1`，单独诊断捕获 `未能完成操作。（UNErrorDomain错误1。）`；程序化触发同一真实通知对象的点击监听器可把窗口从 hidden/unfocused 恢复为 visible/focused，但系统横幅出现与真人点击仍没有证据，详见 `BLOCKED.md` V7。
- [x] 独立差异复核未发现阻断级运行时、类型、竞态、泄漏、IPC 越权或白名单越界问题。复核指出通知正文不应暗示持久任务完成，已改为“本轮运行已结束”对应的中英文，与项目的运行结束语义一致。
- [x] 阶段二最终复跑：双语 452/452 零差异，类型检查、主进程/预加载语法、目标 lint、纯逻辑 4/4、既有四条桌面链路锚点、`git diff --check` 均通过；实现完成，准备阶段提交。
- [x] 阶段二提交：`feat: add desktop window and completion controls`；提交后工作区干净。

### 阶段三进度

- [x] 先用一次性工具得到入口包成分表，再只对 mermaid、katex、react-syntax-highlighter 做动态加载；同时在四个新组件测试文件中各补至少 3 个测试用例，总数从 280 提到 292。
- [x] 阶段二与阶段三未交叉：阶段二已先独立提交，才开始拆包与新组件测试。
- [x] 拆包前重建入口为 2,172,158 B。一次性工具 `npx --yes vite-bundle-visualizer -c vite.config.mts -t raw-data -o /tmp/pi-web-stage3-baseline-stats.json --open false` 未改 package/lock；成分数据显示 react-syntax-highlighter 直接贡献 245,279 B（11.29%）、katex 直接贡献 460,201 B（21.19%）、mermaid 本体原已异步因而入口为 0 B；前两者合计占旧入口 32.48%。
- [x] 一次性工具后文件系统重新显现历史“ 2”副本；未再运行分析工具，未删除/提交副本，详见 `BLOCKED.md` V3。
- [x] 正文的公共 Markdown 插件链、代码/图表渲染壳改为按需加载；文件工作区通过两层按需边界延后已冻结的 `FileViewer` 整棵依赖，没有修改 `FileViewer` 本体或构建配置。单层更快，但构建器会把 katex chunk 名写进入口预加载表（717,120 B / katex=1）；为满足硬验收，采用只在用户打开文件时多一次 1,022 B 中间模块请求的双层方案。
- [x] 最终 `npm run build` 通过（2887 modules）；入口 `index-sFcKPkJ_.js` 为 716,976 B（约 0.684 MiB），比本阶段基线减少 1,455,182 B / 66.99%，低于 1.2 MB；源码产物与最终桌面包内入口均为同一字节数，且 katex=0、mermaid=0、react-syntax-highlighter=0。异步产物仍为 FileViewer 约 22,940 B、MarkdownRuntimeLoader 约 570 B、MarkdownCodeBlock 约 260 B、markdown 约 222,750 B、katex 约 259,040 B、MermaidBlock 约 777,610 B。
- [x] 拆包反向验证：临时把代码/图表壳改回静态加载后，构建入口 `index-WSMuK4QT.js` 为 715,598 B，`grep mermaid` 从 0 变为 1（红）；还原按需加载后恢复三项 0/0/0（绿），最终收口构建入口为 716,976 B。
- [x] 新增四个组件测试文件，各 3 个真实静态渲染用例，共 12/12 通过、0 fail/skip/todo；组件测试文件从 4 增到 8。独立复核初次拒绝了测试内重写项目分组和未直接覆盖 toolResult 的弱方案；最终只给既有项目归并纯函数增加测试导出，生产调用与计数流程原样不动，测试直接执行这份生产逻辑，并明确锁住 user/assistant/toolResult 三角色。
- [x] 组件测试反向验证：临时把标签文字节点的样式类改坏后，定向测试退出 1，3 测中 1 过/2 败，两处均明确报节点类名不匹配（红）；原样恢复后四文件 12/12 通过（绿）。
- [x] 阶段三全量检查保持基线：`npm run check` 的 typecheck/lint 通过，主测试为 292 测 / 290 过 / 仅 2 个既定具名失败 / 0 skipped / 0 todo；因 `&&` 停于既定失败后，单独复跑 Pi 适配器 11/11、Electron 纯逻辑 4/4、i18n 452/452 零差异均通过。
- [x] 阶段三收口服务验证：隔离会话/运行数据下以页面 5174 / 接口 30142 启动 `npm run dev`；`node scripts/smoke-api.mjs` 15/15 通过、覆盖 67 路由；同一页面 visual-smoke 在桌面/移动均无水平溢出，页面/控制台/请求错误均为 0，末行 `VISUAL SMOKE PASS`，截图在本任务可视化目录 `stage3/`。
- [x] 富文本真实浏览器亲验最终通过：依赖自带管理器生成的合法隔离会话在同一条助手消息中得到 Mermaid SVG=1、KaTeX=2（块公式=1）、TypeScript 代码=1、高亮标记=23、加载占位=0；控制台/页面/请求/四百以上响应错误均为 0，截图在 `stage3-rich/stage3-rich-render.png`。
- [x] 亲验首次成功时发现图表必须点第二次：稍后完成的数学插件加载使外层重建渲染函数并卸载图表状态。已把渲染函数按真实依赖记忆化；同一浏览器流程复测点击次数=1、其余指标不退化。类型、目标 lint、既有 MarkdownBody 2/2 和差异检查均通过。
- [x] 最终生产构建再次通过：2887 modules，入口 716,976 B，三项扫描 0/0/0；最终主测试 292/290/2（仍仅两项既定失败）、0 skip/todo，适配器 11/11、Electron 4/4、i18n 452/452。
- [x] 最终代码再次执行 API/视觉收口：`node scripts/smoke-api.mjs` 为 15/15、67 路由；5176/30144 隔离开发服务上的 desktop/mobile visual-smoke 水平溢出与三类浏览器错误均为 0，末行 `VISUAL SMOKE PASS`，截图在 `stage3-final/`；服务、端口与一次性目录均已回收。
- [x] 阶段三提交：`perf: split rich rendering and expand component coverage`；最终补丁归并后仍只保留三个阶段提交。业务跟踪差异归零，31 个超出白名单的历史未跟踪重名副本继续保留待裁决。
