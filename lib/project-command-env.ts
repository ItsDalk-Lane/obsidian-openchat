...
/**
 * Isolate per-project bash command environments from host-only runtime variables.
 *
 * When a session runs `bash` inside a project, it should not inherit `PORT`,
 * `NODE_ENV`, or `NEXT_*` from the pi-web host process — those would either be
 * meaningless inside the project shell or actively change tool behaviour.
 * SDK and extension env overrides are preserved by re-applying them on top
 * of the sanitized environment in `createProjectCommandBashOperations`.
 */
