...
import type { ChatMessage, ChatSession } from 'src/features/chat/types/chat';
...
export class SubAgentToolExecutor implements ToolExecutor {
	...
	async execute(
		...
		try {
			...
			if (!assistantMessageRef) {
				assistantMessageRef = this.createInternalMessage('assistant', assistantMessage.content);
				internalMessages.push(assistantMessageRef);
			} else {
				assistantMessageRef.content = assistantMessage.content;
			}
