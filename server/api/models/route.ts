...
async function loadModels(cwd: string): Promise<ModelsData> {
  ...
  // Delegate to pi's resolver so glob patterns (`my-gateway/*`) and `:thinking`
  // pins are honored exactly like the CLI. The previous exact-string filter
  // silently dropped every model behind a glob (#307, #06522eb).
  const scopeResult = await resolveVisibleModels(services.modelRuntime, enabledModels);
  const visible = scopeResult.visible;
