...
export async function POST(req: Request) {
  ...
  try {
    ...
    if (!pkg?.trim()) return ApiResponse.json({ error: "package required" }, { status: 400 });

    const isGlobal = scope !== "project";
    if (!isGlobal) {
      if (!cwd) return ApiResponse.json({ error: "cwd required for project install" }, { status: 400 });
    ...
    if (!success) {
      return ApiResponse.json({ error: output.slice(-300) || "Install failed" }, { status: 500 });
    }
    return ApiResponse.json({ success: true, output });
  } catch (e: unknown) {
    const err = e as { stdout?: string; stderr?: string; message?: string };
    const output = ((err.stdout ?? "") + (err.stderr ?? "")).replace(ANSI_RE, "");
    return ApiResponse.json({ error: output || (err.message ?? String(e)) }, { status: 500 });
  }
}
