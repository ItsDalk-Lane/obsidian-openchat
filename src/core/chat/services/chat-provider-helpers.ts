...
export const getOllamaCapabilities = async (params: {
	...
	baseURL: string;
	model: string;
	requestHttp: Pick<ObsidianApiProvider, 'requestHttp'>['requestHttp'];
	...
	try {
		const response = await params.requestHttp({
			url: `${normalizedBase}/api/show`,
			method: 'POST',
			body: JSON.stringify({ model: params.model }),
		});
		const responseJson = response.json as { capabilities?: unknown } | undefined;
		const capabilities = Array.isArray(responseJson?.capabilities)
...
export const getOllamaCapabilitiesForModel = async (params: {
	...
	const caps = await getOllamaCapabilities({
		...
		baseURL,
		model: modelName,
		requestHttp: params.requestHttp,
	});
