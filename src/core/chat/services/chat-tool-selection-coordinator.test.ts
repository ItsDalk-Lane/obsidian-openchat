...
function createWorkflowTool(name: string, description: string): ToolDefinition {
	return compileExecutableToolDefinition(attachToolSurfaceMetadata({
		name,
		description,
		inputSchema: {
...
test('ChatToolSelectionCoordinator 在关闭 workflow 模式时允许 workflow 工具回到原子候选池', async () => {
...
test('ChatToolSelectionCoordinator 在关闭 workflow 默认隐藏时允许 sub-agent 工具进入候选池', async () => {
	const workflowTool = createWorkflowTool('sub_agent_researcher', '委托研究子代理处理复杂任务。');
	const fakeResolver = {
		async buildDiscoveryCatalog() {
			return buildDiscoveryCatalog({
				tools: [],
