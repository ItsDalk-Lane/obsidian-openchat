...
function phaseLabel(phase: AgentPhase): string {
  if (phase?.kind === "running_tools") {
    ...
    if (names.length === 1) return `运行 ${names[0]}...`;
    if (names.length <= 3) return `运行 ${names.join(", ")}...`;
    return `运行 ${names.slice(0, 2).join(", ")} (+${names.length - 2})...`;
  }
  if (phase?.kind === "waiting_model") return "等待模型响应...";
  if (phase?.kind === "running_command") return "运行命令中...";
  return "思考中...";
}
        ...
        <div ref={scrollContainerRef} className="flex-1 overflow-y-auto pt-4 [scrollbar-width:none]">
          <div style={{ padding: `0 ${CHAT_COLUMN_PADDING}px` }}>
            ...
            {bashRunning && !pendingBash && (
              <div className="py-2 text-[13px] text-text-muted">
                <span className="animate-[pulse_1.5s_infinite]">运行命令中...</span>
              </div>
            )}
