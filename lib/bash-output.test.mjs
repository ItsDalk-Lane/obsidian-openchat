...
test("accepts only pi bash logs directly inside the configured temp directory", async () => {
  ...
  const validPath = join(tempRoot, "pi-bash-ab12.log");

  assert.equal(
    resolveBashOutputPath(validPath, tempRoot),
    validPath,
  );
  ...
  try {
    ...
    try {
      ...
      if (error && typeof error === "object" && "code" in error && error.code === "EPERM") {
        t.diagnostic("Skipping symlink assertion because this environment does not allow creating symlinks");
        return;
      }
      throw error;
    }
