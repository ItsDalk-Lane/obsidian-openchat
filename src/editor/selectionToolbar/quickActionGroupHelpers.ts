...
/**
 * 计算快捷操作的嵌套层级（从 QuickActionDataService 提取的纯函数）
 */
export function getNestingLevelSync(quickActionId: string, quickActions: QuickAction[]): number {
	let level = 0;
 ...
 * 在快捷操作列表中查找指定操作的父分组
 */
export function findParentGroupSync(quickActionId: string, quickActions: QuickAction[]): QuickAction | null {
...
/**
 * 从所有分组的 children 中移除指定操作（就地修改）
 */
export function removeFromAllGroupsSync(quickActionId: string, quickActions: QuickAction[]): void {
	for (const quickAction of quickActions) {
 ...
 * 递归计算子树的最大相对深度
 */
export function getSubtreeMaxRelativeDepthSync(quickActionId: string, quickActions: QuickAction[]): number {
...
/**
 * 重新排列顶层快捷操作顺序，可选将指定操作移到目标位置（就地修改）
 */
export async function reorderTopLevelQuickActionsSync(quickActions: QuickAction[], movingQuickActionId?: string, position?: number): Promise<void> {
