...
- [x] 阶段一提交：`cf2ce8c style: unify settings dialog visuals`；提交后工作区干净。
...
- [待亲验] 自动化无法可靠操作桌面菜单与系统通知；最终桌面清单明确保留：`CmdOrCtrl+N` 实际打开新会话、后台运行结束通知及点击恢复/聚焦、拖动缩放/最大化后重启恢复。不把启动成功冒充为这三项人眼交互已通过。
