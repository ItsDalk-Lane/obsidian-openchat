...
import {
	buildDirectoryTree,
	collectVaultFilePaths,
} from './filesystemFileOps';

export const executeListDirectory = (
	app: App,
	...
	if (view === 'vault') {
		...
		return asStructuredOrText(response_format, payload, (structured) => {
	...
	const basePayload = {
		...
		items: pagedEntries.map((entry) => ({
			...
			...(include_sizes
				? {
					size: entry.size,
					sizeText: entry.type === 'directory' ? null : formatSize(entry.size),
				}
				: {}),
		})),
		meta: {
	...
	if (!include_sizes) {
		return asStructuredOrText(response_format, basePayload, (structured) => {
			...
			return [
				...
				...(meta.truncated ? ['[结果已截断，请增大 limit 或调整 offset]'] : []),
			].join('\n');
		});
	}
				...
				...items.map((entry) =>
					`${entry.type === 'directory' ? '[DIR]' : '[FILE]'} ${entry.name.padEnd(30)} ${
						entry.type === 'directory'
							? ''
							: String(entry.sizeText ?? '').padStart(10)
					}`.trimEnd(),
				),
				'',
