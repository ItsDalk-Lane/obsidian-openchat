...
import type { BaseOptions, ProviderSettings, SaveAttachment } from 'src/types/provider';
...
export class ChatService {
	...
	private getAiRuntimeToolSettings() {
		return {
			globalTools: [],
...
• 其他包含 "image" 的模型`, 10000);
	...
	async sendMessage(content?: string) {
		...
		const provider = this.resolveProvider();
		if (!provider) {
			new Notice('尚未配置任何AI模型，请先在AI Runtime 设置中添加Provider。');
			return;
		}
	...
	async refreshProviderSettings(aiRuntimeSettings: AiRuntimeSettings) {
		if (!aiRuntimeSettings.providers.length) {
			...
			this.state.selectedModelId = aiRuntimeSettings.providers[0].tag;
			if (this.state.selectedModels.length === 0) {
				this.state.selectedModels = [aiRuntimeSettings.providers[0].tag];
			}
		} else {
			const providerTags = new Set(aiRuntimeSettings.providers.map((provider) => provider.tag));
	...
	private getDefaultProviderTag(): string | null {
		return this.plugin.settings.aiRuntime.providers[0]?.tag ?? null;
	}
	...
	async generateAssistantResponseForModel(
		...
		const providerOptions: BaseOptions = {
			...
			parameters: (providerOptionsRaw.parameters as Record<string, unknown> | undefined) ?? {},
		};
...
3. 尝试在设置中更改图片保存位置`);
	...
	resolveProviderByTag(tag?: string): ProviderSettings | null {
		const providers = this.plugin.settings.aiRuntime.providers;
	...
	findProviderByTagExact(tag?: string): ProviderSettings | null {
		...
		return this.plugin.settings.aiRuntime.providers.find((provider) => provider.tag === tag) ?? null;
	}
	...
	getAiRuntimeSettingsSnapshot(): AiRuntimeSettings {
		return this.cloneValue(this.plugin.settings.aiRuntime);
	}
	...
	async persistGlobalSystemPromptsEnabled(enabled: boolean): Promise<void> {
		const previousAiRuntimeSettings = this.cloneValue(this.plugin.settings.aiRuntime);
		this.plugin.settings.aiRuntime.enableGlobalSystemPrompts = enabled;

		try {
			await this.plugin.saveSettings();
		} catch (error) {
			this.plugin.settings.aiRuntime = previousAiRuntimeSettings;
			this.handleSettingsSaveError(error);
			throw error;
		}
	}
	...
	async persistMcpSettings(mcpSettings: McpSettings): Promise<void> {
		const previousAiRuntimeSettings = this.cloneValue(this.plugin.settings.aiRuntime);
		this.plugin.settings.aiRuntime.mcp = this.cloneValue(mcpSettings);
		syncToolExecutionSettings(this.plugin.settings.aiRuntime);
	...
	async executeSubAgentCommand(agentName: string, task?: string): Promise<void> {
		...
		if (agent.metadata.models?.trim()) {
			...
			if (targetVendor) {
				const matchingProvider = this.plugin.settings.aiRuntime.providers.find((candidate) => {
					return candidate.tag === modelTag || candidate.options.model === modelTag;
				});
				this.state.selectedModelId = matchingProvider?.tag ?? modelTag;
			}
		}

		const resolvedModelTag = this.state.selectedModelId ?? this.getDefaultProviderTag();
		if (!resolvedModelTag) {
			new Notice('尚未配置任何AI模型，请先在AI Runtime 设置中添加Provider。');
			return;
		}

		await this.generateAssistantResponseForModel(prepared.session, resolvedModelTag, {
