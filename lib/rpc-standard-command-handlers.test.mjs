...
test("reload syncs project trust before loading resources", async () => {
  const { dispatchStandardCommand } = await jiti.import("./rpc/standard-command-handlers.ts");
  const calls = [];
  const context = createContext({
    inner: {
    ...
    waitForExtensionsBound: async () => { calls.push("bound"); },
    syncProjectTrust: () => { calls.push("trust"); },
    extensionUi: {
      clearDecorations: () => { calls.push("clear"); },
    },
    applyForcedEmptySystemPrompt: () => { calls.push("prompt"); },
  });

  await dispatchStandardCommand({ type: "reload" }, context);
  assert.deepEqual(calls, ["bound", "trust", "clear", "reload", "prompt"]);
});
