...
test("kernel event decoder maps unknown events to diagnostic event", () => {
  const taskId = createTaskId("task");
  const runId = createRunId("run");
  const decoded = decodeKernelEvent({ type: "totally_new", foo: 1 }, { taskId, runId, sessionId: "s1" });
  assert.equal(decoded.type, "native.diagnostic");
  assert.equal(decoded.payload.nativeType, "totally_new");
});
