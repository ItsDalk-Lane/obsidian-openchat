 ...
 *
 * Sessions from worktrees of one repository share a single project entry because
 * they share the same projectRoot after the server-side worktree grouping.
 * Falls back to the session's raw cwd for non-git directories.
 */
export function getRecentProjects(sessions: readonly SessionInfo[]): RecentProject[] {
  ...
  for (const session of sessions) {
    ...
    if (!root) continue;
    const key = (session as { projectKey?: string }).projectKey ?? root;
...
export function getProjectActivity(
  ...
  for (const session of sessions) {
    const key = (session as { projectKey?: string }).projectKey ?? session.projectRoot ?? session.cwd;
    if (!key) continue;
...
export function sessionsForProject(
  ...
  return sessions.filter(
    (session) => ((session as { projectKey?: string }).projectKey ?? session.projectRoot ?? session.cwd) === projectKey,
  );
}
