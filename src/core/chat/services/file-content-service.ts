...
import { FILE_EXTENSION_LANGUAGE_MAP } from './file-content-language-map';
...
export class FileContentService {
	...
	constructor(
		private readonly obsidianApi: Pick<
			ObsidianApiProvider,
			'getVaultEntry' | 'listFolderEntries' | 'readVaultFile' | 'statPath'
		>,
	) {}
	...
	async readFileContent(file: SelectedFile, options?: FileContentOptions): Promise<FileContent | null> {
		const opts = { ...this.defaultOptions, ...options } as Required<FileContentOptions>;
		
		try {
			const entry = this.obsidianApi.getVaultEntry(file.path);
			if (!entry || entry.kind !== 'file') {
				DebugLogger.warn(`[FileContentService] 文件不存在或不是文件: ${file.path}`);
				return null;
			}

			const fileSize = await this.obsidianApi.statPath(file.path);
			...
			let content = await this.obsidianApi.readVaultFile(file.path);
	...
	async readFolderContent(folder: SelectedFolder, options?: FileContentOptions): Promise<FolderContent | null> {
		...
		try {
			const entry = this.obsidianApi.getVaultEntry(folder.path);
			...
			await this.processFolder(entry, files, opts);
...
\`\`\``;
	...
	private async processFolder(
		folder: VaultEntry,
		results: FileContent[],
		options: Required<FileContentOptions>,
	): Promise<void> {
		for (const child of this.obsidianApi.listFolderEntries(folder.path)) {
			...
			if (child.kind === 'file') {
				...
				const selectedFile: SelectedFile = {
					...
					name: child.name,
					path: child.path,
					extension: child.name.split('.').pop() ?? '',
					type: 'file'
				};
	...
	private getLanguageId(extension: string): string {
		return FILE_EXTENSION_LANGUAGE_MAP[extension.toLowerCase()] || 'text';
	}
}
